...
public class SwiftFlutterLiveActivitiesbPlugin: NSObject, FlutterPlugin, FlutterStreamHandler {
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        let channel = FlutterMethodChannel(name: "flutter_live_activitiesb", binaryMessenger: registrar.messenger())
        let eventChannel = FlutterEventChannel(name: "flutter_live_activitiesb/event", binaryMessenger: registrar.messenger())
        
        let instance = SwiftFlutterLiveActivitiesbPlugin()
    ...
    func createActivity(data: [String: String], result: @escaping FlutterResult) {
        let attributes = FlutterLiveActivitiesb()
        let contentState = FlutterLiveActivitiesb.LiveData(data: data)
        
        do {
            let activity = try Activity<FlutterLiveActivitiesb>.request(
    ...
    func updateActivity(liveId: String, data: [String: String], result: @escaping FlutterResult) {
        Task {
            for activity in Activity<FlutterLiveActivitiesb>.activities {
                if liveId == activity.id {
                    let updatedStatus: FlutterLiveActivitiesb.LiveData = .init(data: data)
